...
**This document provides a tutorial for computing state-of-the-art polygenic score and sex-aware ploygenic score utilizing GXwasR R package.**
...
## Load GXwasR and Example Datasets:
...
# Computing Standard PGS using ComputePGS():
...
Now, our discovery data and target data are ready. We will proceed towards computing first sex-combined PGS and then sex-stratified PGS.
...
We will utilize the `ComputePGS()` to perform the steps (A), (B) and (C).
...
**(D) Perform regression to test the association of PGS and sex.**

**(E) Perform the regression using different thresholds of PGS to check for consistent evidence of sex difference.**
...
In this case, the association between PGS and sex seems insignificant. Users can check with different thresholds of PGS as mentioned in step (D).
...
# Computing PGS for female-only target data using male-only discovery data
...
# Computing PGS for male-only target data using female-only discovery data
...
**(F) Perform regression to test the association of PGS for a trait in one sex and case status in other sex.**

**PGS ~ trait + PCs + other covariates**

Let's check the association between PGS and trait in female.
```{r, fig.width=10,fig.height=6}
# Running
# First, we need to make fdata object.
...
x <- SexRegress(fdata, regressor_index, response_index)
x
# In this case, there is a significant association between PGS and trait value.
```
