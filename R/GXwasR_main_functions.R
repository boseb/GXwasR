...
#'
#' @param DataDir A character string for the file path of the all the input files. The default is NULL.
#' @param ResultDir A character string for the file path where all output files will be stored. The default is tempdir().
...
#' @param precomputedLD A dataframe object as LD matrix with columns: "CHR", "SNP", "BP", "ld_size", "MAF", "ld_score". . The default is NULL.
#' @param chi2_thr1 Numeric value for threshold on chi2 in step 1 of LDSC regression. Default is 30.
#' @param chi2_thr2 Numeric value for threshold on chi2 in step 2. Default is Inf (none).
#' @param intercept Numeric value to constrain the intercept to some value (e.g. 1) in LDSC regression. Default is NULL
...
#' @param ncores Integer value, specifying the number of cores to be used for running LDSC model. The default is 2.
...
#' @param prevalence Numeric value, specifying the disease prevalence. The default is NULL. Note: for the continuous trait value, users should use the default.
...
#' @param minMAF Positive numeric value (0,1), specifying the minimum threshold for the MAF filter of the SNPs in the GREML model. This value cannot be greater than 'maxMAF' parameter. The default is NULL. For NULL, maximum MAF value of the genotype data will be computed and printed on the plot.
#' @param maxMAF Positive numeric value (0,1), specifying the maximum threshold for the MAF filter of the SNPs in the GREML model. This value cannot be less than 'minMAF' parameter. The default is NULL. For NULL, minimum MAF value of the genotype data will be computed and printed on the plot.
...
#' @param indepSNPs A dataframe with independent SNP ids with column name "rsid". The default is NULL.
...
#' @param plotjpeg Boolean value, 'TRUE' or 'FALSE', specifying whether to save the plots in jpeg file in ResultDir. The default is TRUE.
#' @param plotname String of character value specifying the name of the jpeg file with the plots. The deafult is "Heritability_Plots".
...
#' @param summarystat A dataframe object with GWAS summary statistics. The mandatory column headers in this dataframe are 'CHR'(Chromosome code), 'BP'(Basepair position), 'A1' (effect allele), ‘SNP’ (i.e., SNP idenitifier), ‘BETA’ or "OR"(i.e., effect-size or logarithm of odds ratio), and ‘P’ (i.e., p-values). Special Notes: The first three columns needed to be "SNP", "A1" and "BETA" or "OR".
...
#' @param DataDir Character string for the file path of the input PLINK binary files.
#' @param ResultDir Character string for the folder path where the outputs will be saved.
#' @param finput Character string, specifying the prefix of the input PLINK binary files with both male and female samples. 
#' This file needs to be in DataDir. Note: Case/control phenotypes are expected to be encoded as 1=unaffected (control), 
#' 2=affected (case); 0 is accepted as an alternate missing value encoding. The missing case/control or quantitative phenotypes 
...
#' @param sex Boolean value, 'TRUE' or 'FALSE' for using sex as covariate in association test. It is applicable genome-wide. 
#' The default is FALSE.
#' @param xsex Boolean value, 'TRUE' or 'FALSE' for using sex as covariate in association test for X-chromosomal SNPs. 
#' The default is FALSE. This will overwrite 'sex' argument for X-chromosome.
#' @param standard_beta Boolean value, 'TRUE' or 'FALSE' in case of quantitative trait for standardizing the trait or phenotype values 
#' (mean 0, unit variance), so the resulting coefficients will be standardized. The default is TRUE.
#' @param xmodel Models "FMcombx01","FMcombx02",and "FMstatrified" can be chosen for both binary and quantitative traits 
...
#' compensation connection between the sexes, FM02 treats men as homozygous females. In the FM01comb and FM01comb 
#' methods, associations are tested separately for males and females with the FM01 and FM02 models, respectively, 
#' and then the combined p values are computed the Fisher's method, Fisher's method with permutation, or Stouffer's 
...
#' @param covartest Vector value with "NULL","ALL" or covarite name/names to be included in the test. The default is NULL. For instance, 
#' the user can choose “AGE” and “SEX” as covartest = c(“AGE”, “SEX”) or all the covariates as covartest = c(“ALL”).
#' @param interaction Boolean value, 'TRUE' or 'FALSE' for including SNP x covariate interaction term/terms from the association analysis. 
...
#' significance is not always determined simply; rather, it depends on the arbitrary coding of the variables. To put it a
#' nother way, you should probably just interpret the p-value for the interaction. Also, The p-values for the covariates 
...
#' @param B Integer value specifying the number of permutation in case of using fisher.method.perm method in stratified GWAS with 
#' FM01comb and FM02comb XWAS models. The default is 10000.
#' @param MF.na.rm Boolean value, 'TRUE' or 'FALSE' for removing p-values of NA in stratified GWAS with FM01comb and FM02comb XWAS 
#' in case of using Fisher’s and Stouffer’s methods. The default is FALSE.
#' @param MF.p.corr Character vector specifying method for correcting the summary p-values for FMfcomb and FMscomb models. Choices 
...
#' @param plotname A character string specifying the prefix of the file for plots. This file will be saved in DataDir. The default is 
#' "GXwas.plot".
#' @param snp_pval Numeric value as p-value threshold for annotation. SNPs below this p-value will be annotated on the plot. The default 
#' is 1e-08.
#' @param annotateTopSnp Boolean value, 'TRUE' or 'FALSE. If TRUE, it only annotates the top hit on each chromosome that is below the 
#' snp_pval threshold. The default is FALSE.
#' @param suggestiveline The default is 5 (for p-value 1e-05).
...
#' Title: This function combines the p-values of two separate GWAS summary statistics (for instance male and female populations), merges them, and then applies various statistical methods (like Stouffer's method, Fisher's method) to integrate the p-values. It also includes functionality for generating plots like Manhattan plots and Q-Q plots.
#'
#' @param SumstatMale R dataframe object of summary statistics of male GWAS with five mandatory columns: "CHR" (numeric chromosome code),"SNP" (variant id), "A1" (allele) ,"POS" (base-pair position) and "P" (p-value). Other coulmns may present.
#' @param SumstatFemale R dataframe object of summary statistics of female GWAS with five mandatory columns: "SNP", "A1", "TEST" , "POS" and "P". Other coulmns may present.
...
#' @param MF.zero.sub Small numeric value for substituting p-values of 0 in GWAS summary statistics. The default is 0.00001. As log(0) results in Inf this replaces p-values of 0 by default with a small float.
#' @param MF.na.rm Boolean value, 'TRUE' or 'FALSE' for removing p-values of NA in stratified GWAS summary satistics in case of using Fisher’s and Stouffer’s methods. The default is TRUE.
#' @param MF.mc.cores Number of cores used for fisher.method.perm for combining p-values. The default is 1.
#' @param B Integer value specifying the number of permutation in case of using fisher.method.perm method. The default is 10000.
#' @param plot.jpeg Boolean value, 'TRUE' or 'FALSE' for saving the plots in .jpeg file. The default is TRUE.
...
#' @param PlotDir A character string specifying the path of the directory where the plots will be saved. The default is tempdir().
#' @param snp_pval Numeric value as p-value threshold for annotation. SNPs below this p-value will be annotated on the plot. The default is 1e-08.
...
#' @param suggestiveline Numeric value for suggestive cut-off line in GWAS manhattan plot. The default is 5 (for p-value 1e-05).
#' @param genomewideline Numeric value for genome-wide significant cut-off line in GWAS manhattan plot. The default is 7.3 (for p-value 5e-08).
#' @param ncores Integer value, specifying the number of cores for parallel processing. The default is 0 (no parallel computation).

#' @return dataframe with GWAS summary statistics (with XWAS for X-chromosomal variants) along with Manhattan and Q-Q plots.
#' @export
#'
#' @references (1) Purcell et. al.,(2007). PLINK: A Tool Set for Whole-Genome Association and Population-Based Linkage Analyses. The American Journal of Human Genetics, 81(3), 559-575. https://doi.org/10.1086/519795.
...
## New function in V7
#' Execute PLINK to Calculate Minor Allele Frequencies (MAF)
#'
...
#'
#' @param DataDir Character. Directory containing the input PLINK files (.bed, .bim, .fam).
#' @param ResultDir Character. Directory to store the output files generated by PLINK.
#' @param finput Character. Base name of the PLINK input files (without extensions).
