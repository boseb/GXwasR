...
performLDClumping <- function(ldclump, DataDir, ResultDir, LDreference, summarystat, clump_p1, clump_p2, clump_r2, clump_kb, byCHR) {
    if (ldclump) {
        ...
        write.table(clumpedResult$BestClump$INDEX, file = normalizePath(file.path(ResultDir, "Valid.SNP"), mustWork = FALSE), quote = FALSE, row.names = FALSE)
...
## Function 99
## Added in 3.0
validateGXwasInputs <- function(DataDir,
    ResultDir,
    finput,
    trait,
    standard_beta,
    ...
    validateInphenocov <- function(param,
    covarPath,
    interaction,
    sex) {
        if (!interaction) {
